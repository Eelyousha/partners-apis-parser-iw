...
class SSPPartnerB(JSONPartner):
    """SSP Partner B - JSON API with dynamic token auth."""
    ...
    async def _get_auth_token(self, session: ClientSession) -> str:
        ...
        auth_url = "https://ssp-partner-b.example/auth"
        data = {
            "login": self.config.partner_b_ssp_login,
            "password": self.config.partner_b_ssp_password,
        }
    ...
    def get_urls(self, start_date: str, end_date: str) -> list[str]:
        user_id = self.config.partner_b_ssp_user_id
        return [
            f"https://ssp-partner-b.example/users/{user_id}/report?start_date={start_date}&end_date={end_date}"
        ]
    ...
    def parse_json(self, data: dict[str, Any]) -> list[tuple[str, int, float]]:
        ...
        net_payable = total_data.get("net_payable_data", {})

        for date_str in dates:
            imps = int(float(count_imps.get(date_str, 0)))
            spent = float(net_payable.get(date_str, 0))
